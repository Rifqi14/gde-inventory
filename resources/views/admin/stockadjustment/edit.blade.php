                    ...
                    <div class="card">
                        ...
                        <div class="card-footer text-right">
                            ...
                            <button type="button" onclick="onSubmit('rejected')" class="btn bg-danger btn-labeled legitRipple text-sm">
                                <b><i class="fas fa-times-circle"></i></b>
                                Reject
                            </button>
                            <button type="button" onclick="onSubmit('approved')" class="btn bg-info btn-labeled legitRipple text-sm">
        ...
        if (products.length > 0) {
            ...
            $.each(products, function (index, value) { 
                ...
                if (isSerial) {
                    icon = 'fas fa-check';
                    badge = 'badge-info';
                    event = `onclick="showSerial(${productID})"`;                    

                    if(serials){
                        serial = [];
                        lastKey = lastSerial;
                        $.each(JSON.parse(serials), function (index, value) {                                                         
                            serial.push({
                                number : value.number,
                                uom    : value.uom,
                                key    : value.key   
                        ...
                        addDataProduct({
                            ...
                            last_key      : lastKey,
                            stockwareouse : parseInt(stockWarehouse),                            
                            sku           : sku,
                            serials       : serial
                        });
                    }else{
                ...
                html += `<tr class="product-item" data-product-id="${productID}" data-product="${product}" data-uom-id="${uomID}" data-qty-before="${currentStock}">                                    
                            ...
                            <td class="text-right" width="30">${isSerial==true?stockWarehouse:currentStock}</td>
                            <td width="30">
                            ...
                            <td class="text-center" width="10">
                                <button class="btn btn-md text-xs btn-warning btn-flat legitRipple ${isSerial==false?'disabled':''}" type="button" ${event}><i class="fa fa-bars"></i></button>
