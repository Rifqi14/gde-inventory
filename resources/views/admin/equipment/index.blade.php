...
<button type="button" id="add-equipment" class="btn btn-labeled text-sm btn-sm btn-success btn-flat legitRipple" onclick="windowLocation('{{ route('equipment.create') }}')">
  <b><i class="fas fa-plus"></i></b> Create
</button>
@endif
@if (in_array('read', $actionmenu))
<button type="button" id="filter-equipment" class="btn btn-labeled text-sm btn-sm btn-default btn-flat legitRipple" onclick="filter()">
      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body table-responsive p-0">
            <table id="table-equipment" class="table table-striped datatable" width="100%">
              <thead>
                <tr>
                  <th width="2%">No.</th>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label for="equipment" class="control-label">Asset</label>
                <input type="text" name="equipment" id="equipment" class="form-control" placeholder="Asset">
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-group">
...
<script>
  ...
  const edit = (id) => {
    document.location   = `{{ route('equipment.index') }}/${id}/edit`;
  }
  ...
  $(function() {
    $('.select2').select2();
    dataTable = $('#table-equipment').DataTable( {
      processing: true,
      language: {
      ...
      order: [[ 1, "asc" ]],
      ajax: {
        url: "{{route('equipment.read')}}",
        type: "GET",
        data:function(data){
          var asset     = $('#form-search').find('input[name=asset]').val();
          var area_id   = $('#form-search').find('select[name=area_id]').val();
      ...
      columnDefs:[
        ...
        { width: "15%",
          render: function ( data, type, row ) {
            return row.area.site ? row.area.site.name : '-';
          }, targets: [3],
        },
        {   
          width: "10%",
