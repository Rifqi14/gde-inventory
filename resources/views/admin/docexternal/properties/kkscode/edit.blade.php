...
<section class="content" id="content">
  <div class="container-fluid">
    <form action="{{ route('kkscode.update', ['id' => $kkscode->id]) }}" method="post" role="form" enctype="multipart/form-data" autocomplete="off" id="form">
      @csrf
      @method('PUT')
      <div class="card">
        <div class="card-body">
          ...
          <div class="row">
            ...
            <div class="col-md-8">
              <div class="form-group row">
                ...
                <input type="text" name="code" id="code" class="form-control col-md-8" placeholder="Code" value="{{ $kkscode->code }}">
              </div>
            </div>
            <div class="col-md-8">
              <div class="form-group row">
                <label for="name" class="control-label col-md-3">Name</label>
                <input type="text" name="name" id="name" class="form-control col-md-8" placeholder="Name" value="{{ $kkscode->name }}">
              </div>
            </div>
          </div>
        </div>
  ...
  $(function() {
    ...
    @if ($kkscode->category)
      $('#kks_category_id').select2('trigger', 'select', {
        data: {
          id: `{{ $kkscode->category->id }}`,
          text: `{{ $kkscode->category->code }}`,
          name: `{{ $kkscode->category->name }}`,
        }
      });
    @endif
  });
