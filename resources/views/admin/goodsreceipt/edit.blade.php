...
<section id="content" class="content">
  <div class="container-fluid">
    <form action="{{ route('goodsreceipt.update', ['id' => $data->id]) }}" id="form" enctype="multipart/form-data">
      {{ csrf_field() }}
      @method('PUT')
      <div class="row">
        <div class="col-md-8">
              ...
              <!-- Receipt Number -->
                ...
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <div class="controls">
                      <select name="site" id="unit" class="form-control select2" data-placeholder="Choose site"></select>
                    </div>
                  </div>
                </div>
                <!-- Warehouse -->
...
<script>
  ...
  $(function() {
    var receiptDate = '{{$data->date_receipt}}',
        siteID      = {{$data->site_id}},
    ...
    if(receiptDate){
      var date = $('#receipt-date').data('daterangepicker');
          date.setStartDate(`${receiptDate}`);
          date.setEndDate(`${receiptDate}`);
    }
      ...
      columnDefs: [{
        ...
        },
        {
          render: function(data, type, row) {
            ...
            return `<button class="btn btn-sm text-xs btn-success btn-flat legitRipple" onclick="addProduct($(this),'borrowing')" type="button" data-reference-id="${referenceID}" data-reference="${reference}" data-product-id="${productID}" data-product="${product}" data-uom-id="${uomID}" data-order="${order}">
                      <i class="fas fa-plus"></i>
                    </button>`;
          },
          targets: [7]
    ...
    $("#form").validate({
      ...
      submitHandler: function() {
        var data        = new FormData($('#form')[0]),
            ...
            siteID      = $('#unit').find('option:selected').val(),
        ...
        $.each($('#table-document > tbody > .document-item').find('.doc-cell'), function (index, val) { 
           var input      = $(this).parents('.document-item').find('.document-name'),
               ...
               transferID = input.attr('data-receipt-id'),
  ...
  const initData = () => {
    ...
    console.log({initProduct : products});
        ...
        var productID    = value.product_id,
            ...
            html = `<tr class="product-item">
                  ...
                  <td class="text-center" width="30">
                    <input type="number" class="form-control numberfield text-right qty-receipt" placeholder="0" value="${receipt}">
                  </td>
                  <td width="100">
                    <div class="form-group">
                      <div class="controls">
      ...
      var html  = '',
        ...
        $.each(files, function (index, value) { 
            ...
            html += `<tr class="document-item">
                        <td>
                            <input type="text" class="form-control document-name" data-id="${id}" data-receipt-id="${receiptID}" data-type="file" data-file="${file}" placeholder="Enter document name" value="${filename}" required>                                
                        </td>
                        <td class="doc-cell">                       
      ...
      var html  = '',
        ...
        $.each(images, function (index, value) { 
            ...
            html += `<tr class="photo-item">
                        <td>
                        <input type="text" class="form-control document-name" data-id="${id}" data-receipt-id="${receiptID}" data-type="photo" data-file="${file}" placeholder="Enter photo name" value="${filename}" required>                            
                        </td>
                        <td class="doc-cell">          
                            <a href="${path}" target="_blank">
                               <b><i class="fas fa-download"></i></b> Download File
  ...
  const addProduct = (that, type) => {
    ...
    var html = `<tr class="product-item">
                  ...
                  <td width="100">${category}</td>
                  <td width="30" class="text-center">${serial}</td>
                  <td class="text-right" width="20">${order}</td>
  ...
  const removeDoc = (that,id) => {
    var document   = that.closest('.document-item').find('.document-name'),
        ...
        receiptID  = document.attr('data-receipt-id'),
    ...
    if(id){
      deletedDoc.push({
        id : id,            
        path : `assets/goodsreceipt/${receiptID}/document/${file}`
      });
    }
  }
  ...
  const removePhoto = (that,id) => {
    ...
    if(id){
      deletedDoc.push({
          id : id,            
          path : `assets/goodsreceipt/${transferID}/image/${file}`
      });  
    }
  }   
